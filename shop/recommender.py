...
# Nawiąanie połączenia z bazą Redis
...
class Recommender:
    ...
    def suggest_products_for(self, products, max_results=6):
        ...
        if len(products) == 1:
            ...
            # Wygenerowanie tymczasowego klucza
            flat_ids = ''.join([str(id) for id in product_ids])
            tmp_key = f'tem_(flat_ids)'
            ...
            # Usunięcie identyfikatorów produktów, dla których przygotowujemy rekomendację
            r.zrem(tmp_key, *product_ids)
            # Pobranie identyfikatorów produktów według ich punktacji, w kolejności malejącej
            suggestions = r.zrange(tmp_key, 0, -1, desc=True)[:max_results]
            # Usunięcie tymczasowego klucza
            r.delete(tmp_key)
        suggested_products_ids = [int(id) for id in suggestions]
        # Pobranie sugerowanych produktów i ułożenie ich w kolejności pojawiania się
